...
function rendererWebGPU(p5, fn) {
  ...
  class StorageBuffer {
    constructor(
      buffer, size, renderer, schema = null, arrayType = Float32Array
    ) {
  ...
  class RendererWebGPU extends Renderer3D {
    ...
    updateUniformValue(shader, uniform, data) {
      if (uniform.isSampler) {
        ...
        const parsedStorage = shader._storageBuffers
          .find((s) => s.name === uniform.name);
...
`;
    ...
    _checkStorageElementType(parsedStorage, storageBuffer) {
      ...
      const elementType = parsedStorage.elementType
        .replace(/^atomic<(\w+)>$/, '$1');
    ...
    createStorage(dataOrCount) {
      ...
      if (typeof dataOrCount === 'number') {
        ...
        if (ArrayBuffer.isView(dataOrCount) &&
      ...
      const storageBuffer = new StorageBuffer(
        buffer, size, this, null, ArrayType
      );
